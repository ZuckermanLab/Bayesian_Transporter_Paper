...
# def log_probability(theta, y_obs):
#     '''log of estimated posterior probability'''
#     logp = log_prior(theta)
#     tmp_model = initialize_model(theta)
#     if not np.isfinite(logp):
#         return -np.inf  # ~zero probability
#     return logp + log_likelihood(theta, y_obs, tmp_model)  # log posterior ~ log likelihood + log prior
...
def main():
    ...
    time_0 = time.time()
    new_dir = pathlib.Path(pathlib.Path.cwd(), f'TEST_{time_str}_emcee_transporter')
    ...
    if parallel:
        with mp.Pool(processes=20) as pool:
            ...
            # sampler = emcee.EnsembleSampler(n_walkers, n_dim, log_probability,  args=(y_obs,model),
            #                             pool=pool, 
            #                             moves=[(emcee.moves.KDEMove(),1.0)])

            sampler = emcee.EnsembleSampler(n_walkers, n_dim, log_probability,  args=[y_obs],
                                         pool=pool, 
                                         moves=[(emcee.moves.KDEMove(),1.0)])
            sampler.run_mcmc(p0[0], n_steps, progress=True)
    ...
    flat_lnlike = sampler.get_log_prob(flat=True)
