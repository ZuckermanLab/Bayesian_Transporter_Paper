...
def calc_log_like(K,y_obs,m):
    ...
    try:
        # D_tmp = m.simulate(0, 10, 250, selections=['time', 'rxn4'])
        D_tmp4 = m.simulate(0, 5, 125, selections=['time', 'rxn4'])
        y_tmp4 = D_tmp4['rxn4'][1:]  # remove first point
        #log_like_tmp = calc_norm_log_like(y_tmp1,sigma,y_obs)
    except:
        # y_tmp = np.zeros(249)  # remove first point
        # print("error:")
    ...
    try:
        # D_tmp = m.simulate(0, 10, 250, selections=['time', 'rxn4'])
        D_tmp5 = m.simulate(0, 5, 125, selections=['time', 'rxn4'])
        y_tmp5 = D_tmp5['rxn4'][1:]  # remove first point
        #log_like_tmp = calc_norm_log_like(y_tmp1,sigma,y_obs)
    except:
        # y_tmp = np.zeros(249)  # remove first point
        # print("error:")
    ...
    y_pred = np.hstack([y_tmp1, y_tmp2, y_tmp3, y_tmp4, y_tmp5])
...
def plot_samples(samples, p_info, beta, run_config):
    ...
    plt.tight_layout()
    plt.savefig(f'TEST_affine_{K_walkers}w_{N_steps}s_{seed}r_data_5exp.png') 
...
if __name__ == "__main__":    
    ...
    m.integrator.relative_tolerance = 1e-12
    m.S_out = 0.1e-3
    D4 = m.simulate(0, 5, 125, selections=['time', 'rxn4'])
    y_true4 = D4['rxn4'][1:]  # remove first point
    ...
    m.S_out = 2e-3
    m.H_out = 5e-7
    D5 = m.simulate(0, 5, 125, selections=['time', 'rxn4'])
    y_true5 = D5['rxn4'][1:]  # remove first point

    y_true = np.hstack([y_true1, y_true2, y_true3, y_true4, y_true5])
    ...
    #np.savetxt("data_grid_test3_5exp_v2.csv", y_obs)
    y_obs = np.genfromtxt("data_grid_test3_5exp_v2.csv")  # 4x experiment
    ...
    df.to_csv(f'TEST_ffine_{K_walkers}w_{N_steps}s_{seed}r_data_5exp.csv')
