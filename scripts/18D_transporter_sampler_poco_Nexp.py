...
def calc_log_like(K,y_obs,m):#m):
    ...
    for i in range(len(H_out_list)):
        ...
        # k6 and k7 and  have cycle constraints
        m.k6_f = 10**K[10]
...
def calc_log_prior(p):
    ...
    lb = np.array([6, -1, -2, -2, 3, -1, -1, 6, -2, -2, -1, 6, -1, 3, 9, 5, 12]) + b  # log10 rate constant prior lower bound + shift
    ub = np.array([12, 5, 4, 4, 9, 5, 5, 12, 4, 4, 5, 12, 5, 9, 3, -1, 6]) + b  # log10 rate constant prior upper bound + shift
...
def synthesize_data(K,m):
    ...
    for i in range(len(H_out_list)):
        ...
        m.k7_r = (m.k2_f*m.k3_f*m.k4_f*m.k5_f*m.k7_f*m.k8_f)/(m.k2_r*m.k3_r*m.k4_r*m.k5_r*m.k8_r)
        m.k9_f = 10**K[14]
        m.k9_r = (m.k10_f*m.k9_f*m.k4_r*m.k3_r)/(m.k10_r*m.k4_f*m.k3_f)

        try:
            D_tmp = m.simulate(0, 50, 125, selections=['time', 'rxn4', 'rxn10'])
            y_tmp = D_tmp['rxn4'][1:] + D_tmp['rxn10'][1:] # remove first point
        except:
...
if __name__ == "__main__":
    ...
    model_file = "/Users/georgeau/Desktop/GitHub/Bayesian_Transporter/transporter_model/antiporter_18D_model.txt"
    obs_data_file = "/Users/georgeau/Desktop/GitHub/Bayesian_Transporter/synthetic_data/synth_data_15D_c1_1expA_125s_v3.csv"
    parameter_file = "/Users/georgeau/Desktop/GitHub/Bayesian_Transporter/transporter_model/18D_transporter_c1234_w_full_priors.json"
    ...
    r = te.loadAntimonyModel(antimony_string_SS)
    ...
    np.savetxt('synth_data_18D_test.csv', y_obs, delimiter=',')
