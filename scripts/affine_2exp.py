...
def calc_log_like(K,y_obs,m):
    ...
    try:
        ...
        # log_like_tmp = calc_norm_log_likelihood(y_tmp,sigma,y_obs)
        log_like_tmp = -np.inf
    y_pred = np.hstack([y_tmp1, y_tmp2])
...
def plot_samples(samples, p_info, beta, run_config):
    ...
    plt.tight_layout()
    plt.savefig(f'affine_{K_walkers}w_{N_steps}s_{seed}r_data_2exp.png') 
...
if __name__ == "__main__":    
    ...
    #np.savetxt("data_grid_test3_2exp_v2.csv", y_obs)
    #y_obs = np.genfromtxt("data_grid_test3_2exp_v2.csv")  # single experiment


    plt.figure(figsize=(10,10))
    plt.plot(y_obs, 'o', alpha=0.5)
    plt.plot(y_true)
    plt.ylim(-7.5e-11, 7.5e-11)
    plt.ylabel('y')
    plt.xlabel('t')
    ...
    df.to_csv(f'affine_{K_walkers}w_{N_steps}s_{seed}r_data_2exp.csv')
