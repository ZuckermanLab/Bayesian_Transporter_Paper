...
def calc_log_like(K,y_obs,m):
    ...
    y_pred = np.hstack([y_tmp1, y_tmp2, y_tmp3, y_tmp4])
...
def plot_samples(samples, p_info, beta, run_config):
    ...
    plt.tight_layout()
    plt.savefig(f'affine_{K_walkers}w_{N_steps}s_{seed}r_data_4exp.png') 
...
if __name__ == "__main__":    
    ...
    y_true = np.hstack([y_true1, y_true2, y_true3, y_true4])
    ...
    #np.savetxt("data_grid_test3_4exp_v2.csv", y_obs)
    y_obs = np.genfromtxt("data_grid_test3_4exp_v2.csv")  # 4x experiment
    ...
    df.to_csv(f'affine_{K_walkers}w_{N_steps}s_{seed}r_data_4exp.csv')
